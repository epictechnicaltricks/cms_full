...
if(isset($_POST["id"]) && !empty($_POST["id"])){
 ...
 if(empty($input_address)){
    ...
    if(empty($name_err) && empty($subject_err) && empty($branch_err) && empty($email_err) && empty($phone_err) && empty($salary_err) && empty($address_err)){
        ...
        $sql ="UPDATE teacher SET name=?, subject=?, branch=?, email=?, phone=?,salary=?, address=? WHERE id=?";

        if($stmt = mysqli_prepare($link,$sql)){
            // bind variables to prepare statement as parameters
            mysqli_stmt_bind_param($stmt,"ssssssi", $param_name, $param_subject, $param_branch, $param_email, $param_phone, $param_salary,$param_address, $param_id);
